...
#          pass
      ...
      #detect for mouse control #x:246 [15]/173 [9] y:  159 [12]/ 145 [4]
      #x
      MouseMoveX = 0
      MouseMoveY = 0
      MovePixel = 1

      right_eye_x_correction = 0.30
      right_eye_width =  ((right_eye[9].x - right_eye[15].x) * right_eye_x_correction)
      right_eye_x_distance = (right_iris_center_x - right_eye_center_x) - kalibraceX
      
      temp = right_eye_width / (2 * 3) # 2 = half of eye; 3 = 3 parts of eye
      if temp > abs(right_eye_x_distance):
        #print('x: idle')
        printTextX = 'x: idle'
      elif temp * 2 > abs(right_eye_x_distance):
        #print('x: Speed I - slow')
        if right_eye_x_distance > 0:
          printTextX = 'x: Speed I - slow - right'
          MouseMoveX = MovePixel
        else:
          printTextX = 'x: Speed I - slow - left'
          MoseMoveX = - MovePixel
      else:
        #print('x: Speed II fast')
        if right_eye_x_distance > 0:
          printTextX = 'x: Speed II fast - right'
          MouseMoveX = MovePixel * 2
        else:
          printTextX = 'x: Speed II fast - left'
          MouseMoveX = - MovePixel * 2
      ...
      if temp > abs(right_eye_y_distance):
        #print('y: idle')
        printTextY = 'y: idle'
      elif temp * 2 > abs(right_eye_y_distance):
        #print('y: Speed I - slow')
        if right_eye_y_distance < 0:
          printTextY = 'y: Speed I - slow - down'
          MouseMoveY = MovePixel
        else:
          printTextY = 'y: Speed I - slow - up'
          MouseMoveY = - MovePixel
      else:
        #print('y: Speed II fast')
        if right_eye_y_distance < 0:
          printTextY = 'y: Speed II - fast - down'
          MouseMoveY = MovePixel * 2
        else:
          printTextY = 'y: Speed I - fast - up'
          MouseMoveY = - MovePixel * 2
    ...
    if cv2.waitKey(1) & 0xFF == ord('c'):
      kalibraceX = right_eye_x_distance
      kalibraceY = right_eye_y_distance
    # quet of program
